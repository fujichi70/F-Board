...
$sql = 'SELECT childcare_boards.id, childcare_boards.user_id, childcare_boards.text, childcare_boards.created_at, childcare_boards.deleted_flag, users.name, users.img_path FROM users INNER JOIN childcare_boards ON users.id = childcare_boards.user_id WHERE deleted_flag=0 ORDER BY created_at DESC';
...
<body>
	...
	<div class="category-box">
		<div class="childcare-text"><h1 class="common-text">育児や子育てにお悩みを持つ皆さんで相談しあいましょう！</h1></div>
		<div class="category-img--parts"><img class="category-img" src="img/childcare.jpg" alt="育児画像"></div>
	</div>

	<section id="category">
		<h2 class="common-title">投稿</h2>
		<?php var_dump($_SESSION['category']); ?>
					...
					<?php if (!empty($_SESSION['id'])) :
						if ($_SESSION['id'] === $row['user_id']) : ?>
							<form action="deleted.php" method="post">
								...
								<input type="hidden" name="category" value="childcare">
